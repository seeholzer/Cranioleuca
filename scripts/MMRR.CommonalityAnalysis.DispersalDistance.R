...
#			Produces Figures 5, 6, S2, S3, S7, and Tables 3 & 4. 
...
####	indices for wing length
tmp.mFst = mFst[colnames(mWing),colnames(mWing)]
cols.to.exclude = which(!(colnames(tmp.mFst) %in% c('Macate','Pacar','Quichas','Huamatanga')))
tmp.fst = tmp.mFst
tmp.fst[,-cols.to.exclude] = -999
tmp.fst[-cols.to.exclude,] = -999
tmp.fst[upper.tri(tmp.fst,diag=T)] = NA
indices.with.west.slope.pop.WING =  which(tmp.fst[lower.tri(tmp.fst,diag=F)] == -999)
...
dist.plotdata = data.frame(fst=prep(mFst),geo=prep(mGeo),cost.raw=prep(mCost.raw),cost.masked=prep(mCost.masked))

plotdata = data.frame(fst=prep(mFst),cost.masked=prep(mDist),alt=prep(mAlt),env=prep(mEnv),mass=prep(mMass))

plotdata.WING = data.frame(fst = prep(mFst[colnames(mWing), colnames(mWing)]), wing = prep(mWing))
...
xlabs = c('Geographic Distance (km)','Least-cost Distance - raw (km)','Dispersal Distance (km)')
names(xlabs) = c('geo','cost.raw','cost.masked')

pdf('outfiles/Figure S2.pdf',width=12,height=4,bg='transparent')

#dev.new(width=12,height=4)
mat = matrix(c(1,2,3), nrow=1, ncol=3, byrow=TRUE)
nf = layout(mat=mat,heights=c(4),widths=c(4,4,4))
variables = c('geo','cost.raw','cost.masked')
i = 'cost.masked'
for(i in variables){
...
xlabs = c('Dispersal Distance (km)','Mass Disparity (g)','Plumage Disparity','Elevation Disparity (m)','Environmental Disparity')
names(xlabs) = c('cost.masked','mass','plum','alt','env')

pdf('outfiles/Figure 5.pdf',width=8,height=8,bg='transparent')

#dev.new(width=8,height=8)
mat = matrix(c(1,2,3,4), nrow=2, ncol=2, byrow=TRUE)
nf = layout(mat=mat,heights=c(4,4),widths=c(4,4))
variables = c('cost.masked','mass','alt','plum')

colnames = c('xvar','min','max')
lims = data.frame(matrix(nrow= length(variables),ncol=length(colnames)))
colnames(lims)= colnames
lims[,'xvar'] = variables
lims[lims$xvar == 'cost.masked',c('min','max')] = c(0,3000)
...
for(i in 1:length(predictors)){
	...
	uni.table[i,'r2'] = sprintf("%.2f", round(fit$r.squared,2))
	uni.table[i,'coefficient'] = sprintf("%.2f", round(fit$coefficients[2],2))
	uni.table[i,'tstatistic'] = sprintf("%.2f", round(fit$tstatistic[2],2))
	uni.table[i,'pvalue'] = sprintf("%.2f", round(fit$Fpvalue,2))
	}

colnames(uni.table) = c('','r2','B','t','p')
uni.table[uni.table[,'p'] < 0.01,'p'] = '< 0.01'

var.order = c('DIST','MASS','ELE','ENV','PLUM') 
uni.table = uni.table[match(uni.table[,1],var.order),]

table.path.uni = 'outfiles/Table.MMRR.univariate.all.populations.txt'
...
table.path.multi = 'outfiles/Table.MMRR.multivariate.all.populations.txt'
...
pdf('outfiles/Figure 6.pdf',width=8,height=7, bg='transparent')
...
############################
############################
#Commonality analysis For Wing length. Creates Figure S3
############################
############################

#remove CorBlanca population because don't have wing length for that one
remove = colnames(mFst)[!(colnames(mFst) %in% colnames(mWing))]
mFst.wing = mFst[!(colnames(mFst) %in% remove),!(colnames(mFst) %in% remove)]
mDist.wing = mDist[!(colnames(mDist) %in% remove),!(colnames(mDist) %in% remove)]
mAlt.wing = mAlt[!(colnames(mAlt) %in% remove),!(colnames(mAlt) %in% remove)]
mEnv.wing = mEnv[!(colnames(mEnv) %in% remove),!(colnames(mEnv) %in% remove)]

#Do Commonality Analysis to get Unique, Common, and Total contribution of each predictor variable
data=data.frame(FST = prep.standardize(mFst.wing), DIST = prep.standardize(mDist.wing), ELE = prep.standardize(mAlt.wing), ENV = prep.standardize(mEnv.wing), WING = prep.standardize(mWing))
ca = regr(lm(FST ~ DIST + WING + ELE + ENV,data=data))
...
for (i in 1:nperm){
	rarray= sort(sample(n,sn,replace=F))
	mmFst = mFst.wing[rarray,rarray][lower.tri(mFst[rarray,rarray],diag=F)]
	mmDIST= prep(mDist.wing[rarray,rarray])
	mmELE = prep(mAlt.wing[rarray,rarray])
	mmENV = prep(mEnv.wing[rarray,rarray])
	mmWING =prep(mWing[rarray,rarray])
	
	
	
	comm=regr(lm(mmFst ~ mmDIST + mmWING + mmELE + mmENV))
	boot[i,]=comm$Commonality_Data$CC[c(1:ncombos),1]
	print(i)
}
for (i in 1:ncombos){
...
pdf('outfiles/Figure S3.pdf',width=8,height=7, bg='transparent')
...
cols.to.exclude = which((colnames(mFst) %in% c('Macate','Pacar','Quichas','Huamatanga')))
mFst.noSWslope = mFst[-cols.to.exclude,-cols.to.exclude]

predictors.noSWslope = lapply(predictors, function(x) {
	cols.to.exclude = which((colnames(x) %in% c('Macate','Pacar','Quichas','Huamatanga')))
	x[-cols.to.exclude,-cols.to.exclude]
	})

FST.noSWslope = prep.standardize(mFst.noSWslope)

cols.to.exclude = which((colnames(mDist) %in% c('Macate','Pacar','Quichas','Huamatanga')))
mDist.noSWslope = mDist[-cols.to.exclude,-cols.to.exclude]
DIST.noSWslope = prep.standardize(mDist.noSWslope) 

cols.to.exclude = which((colnames(mAlt) %in% c('Macate','Pacar','Quichas','Huamatanga')))
mAlt.noSWslope = mAlt[-cols.to.exclude,-cols.to.exclude]
ELE.noSWslope = prep.standardize(mAlt.noSWslope)

cols.to.exclude = which((colnames(mEnv) %in% c('Macate','Pacar','Quichas','Huamatanga')))
mEnv.noSWslope = mEnv[-cols.to.exclude,-cols.to.exclude]
ENV.noSWslope = prep.standardize(mEnv.noSWslope)

cols.to.exclude = which((colnames(mMass) %in% c('Macate','Pacar','Quichas','Huamatanga')))
mMass.noSWslope = mMass[-cols.to.exclude,-cols.to.exclude]
MASS.noSWslope = prep.standardize(mMass.noSWslope)

cols.to.exclude = which((colnames(mWing) %in% c('Macate','Pacar','Quichas','Huamatanga')))
mWing.noSWslope = mWing[-cols.to.exclude,-cols.to.exclude]
WING.noSWslope = prep.standardize(mWing.noSWslope)

cols.to.exclude = which((colnames(mPlumage) %in% c('Macate','Pacar','Quichas','Huamatanga')))
mPlumage.noSWslope = mPlumage[-cols.to.exclude,-cols.to.exclude]
PLUM.noSWslope = prep.standardize(mPlumage.noSWslope)
...
colnames(uni.table) = colnames
i = 1
for(i in 1:length(predictors.noSWslope)){
	cat(i,'\n')
		
	tmp.mFst = mFst.noSWslope[colnames(predictors.noSWslope[[i]]),colnames(predictors.noSWslope[[i]])]
			
	fit = MMRR(tmp.mFst, predictors.noSWslope[i],nperm=1000)
	uni.table[i,'predictor'] = names(predictors.noSWslope)[i]
	uni.table[i,'r2'] = sprintf("%.2f", round(fit$r.squared,2))
	uni.table[i,'coefficient'] = sprintf("%.2f", round(fit$coefficients[2],2))
	uni.table[i,'tstatistic'] = sprintf("%.2f", round(fit$tstatistic[2],2))
	uni.table[i,'pvalue'] = sprintf("%.2f", round(fit$Fpvalue,2))
...
table.path.uni = 'outfiles/Table.MMRR.univariate.noSWslope.txt'
...
#MMRR to get regression coefficients and significance values for multivariate model
#data for MMRR
x = predictors.noSWslope[!(names(predictors.noSWslope) %in% c('SIZE','PLUM'))] #can't include plumage, besides, explains almost none of the variance
fit = MMRR(Y = mFst.noSWslope, X = x, nperm=1000)

#Do Commonality Analysis to get Unique, Common, and Total contribution of each predictor variable
data=data.frame(FST = FST.noSWslope, DIST = DIST.noSWslope, ELE = ELE.noSWslope, ENV = ENV.noSWslope, MASS = MASS.noSWslope)
ca = regr(lm(FST ~ DIST + MASS + ELE + ENV,data=data))
...
#this corresponds to the lower table for all populations in Table 4
table.path.multi = 'outfiles/Table.MMRR.multivariate.noSWslope.txt'
write.table(multi.table,file = table.path.multi,quote=F,col.names=T,row.names=F,sep='\t')
...
boot=matrix(data = 0, nrow = nperm, ncol = ncombos)
resBootstrap=data.frame(n=rep(0,ncombos),o=rep(0,ncombos),l=rep(0,ncombos),u=rep(0,ncombos),p=rep(0,ncombos))
n=ncol(mFst.noSWslope)
sn=0.9*n


for (i in 1:nperm){
	rarray= sort(sample(n,sn,replace=F))
	mmFst = mFst.noSWslope[rarray,rarray][lower.tri(mFst[rarray,rarray],diag=F)]
	mmDIST= prep(mDist.noSWslope[rarray,rarray])
	mmELE = prep(mAlt.noSWslope[rarray,rarray])
	mmENV = prep(mEnv.noSWslope[rarray,rarray])
	mmMASS =prep(mMass.noSWslope[rarray,rarray])
...
pdf('outfiles/Figure S7.pdf',width=8,height=7, bg='transparent')
